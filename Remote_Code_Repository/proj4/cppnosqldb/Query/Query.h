...
namespace NoSqlDb
{
	...
	public:
		...
		void children(RegExp regExp) { children_ = regExp; }
		bool matchChildren();
		void categories(RegExp regExp) { categories_ = regExp; }
		bool matchCategories();
		void version(RegExp regExp) { version_ = regExp; }
		bool matchVersion();
	...
	bool Conditions<P>::match()
	{
		try
		{
			return matchName() && matchChildren() && matchCategories() && matchVersion();
		}
	...
	bool Conditions<P>::matchVersion() {
		if (version_ == "")
			return true;
		std::regex re(version_);
		std::regex num("[0-9]+?");
		//check if vresion_ is numeric
		if (!std::regex_match(version_, num)) {
			return false;
		}
		std::string temp = pDbElem_->payLoad().value();
		...
		std::string versionNum = temp.substr(lastDot + 1);
		bool match = regex_match(versionNum, num);
		if ((!match) && version_ == "1") {
			return true;
		}
		else {
			return std::regex_search(versionNum, re);
		}
	}


	template<typename P>
	bool Conditions<P>::matchCategories()
	{
		if (categories_ == "")
			return true;
		//std::regex re(categories_);
		/* return  [this](DbElement<PayLoad>& elem) {
		return (elem.payLoad()).hasCategory(this->categories_);
		};*/
		return  pDbElem_->payLoad().hasCategory(categories_);
	}
	...
	bool Conditions<P>::matchChildren()
	{
		...
		if (std::find(start, end, children_) == end)
			return false;
